package com.amrita.jpl.cys21040.pract.gui;

import javax.swing.*;
import java.awt.*;

/**
 * This class demonstrates the usage of FlowLayout in Java Swing.
 * It creates a simple GUI window with components arranged using FlowLayout.
 *
 * @author G Manomithran
 */
public class FlowLayoutExample extends JFrame {

    /**
     * Constructs the FlowLayoutExample window.
     */
    public FlowLayoutExample() {
        setTitle("FlowLayout Example");
        setSize(600, 80);
//        setSize(400, 220);

        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        // Set the FlowLayout as the layout manager for the frame
        setLayout(new FlowLayout());

        // Create and add components to the content pane

        // Labels
        JLabel label1 = new JLabel(" Full Name");
        JLabel label2 = new JLabel("Roll Number");
        JLabel label3 = new JLabel("Year");
        JLabel label4 = new JLabel("Tell about yourself : ");


        // Buttons
        JTextField A = new JTextField(10);
        JTextField button1 = new JTextField(10);
        JTextField button2 = new JTextField(10);
        JTextField button3 = new JTextField(10);
        JButton B = new JButton("Submit here!!");
        // Add components to the content pane

        add(label1);
        add(button1);
        add(label2);
        add(button2);
        add(label3);
        add(button3,JLabel.SOUTH);
        add(label4);
        add(A);
        add(B);

        // Make the window visible
        setVisible(true);
    }

    /**
     * Main method to launch the application.
     *
     * @param args command-line arguments (unused)
     */
    public static void main(String[] args) {
        new FlowLayoutExample();
    }
}
